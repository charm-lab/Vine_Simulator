...
function solve(x0,prob::MOI.AbstractNLPEvaluator;
        tol=1.0e-6,c_tol=1.0e-6,nlp=:ipopt,max_iter=1000,d_tol=1.)
    ...
    if nlp == :ipopt
        ...
        solver.options["dual_inf_tol"] = d_tol
        solver.options["acceptable_iter"] = Int(2)
        solver.options["acceptable_tol"] = 10*tol
        solver.options["constr_viol_tol"] = 10*c_tol
        solver.options["dual_inf_tol"] = 10*d_tol
    elseif nlp == :snopt
